...
function SinglePlaylist() {
  ...
  const handlePlay = (idx) => {
    ...
    dispatch(addSongToPlayer(songIds[0])).then(() =>
  ...
  if (loaded) {
    return (
      <div>
        ...
        <button className="playlist-buttons" onClick={() => handlePlay()}>
          <FontAwesomeIcon className="fa-solid" icon={faPlay} />
        </button>

        {authorized ? (
          <>
            <button className="playlist-buttons" onClick={() => handleEdit()}>
              <FontAwesomeIcon className="fa-solid" icon={faPenToSquare} />
            </button>
            <button className="playlist-buttons" onClick={() => handleDelete()}>
