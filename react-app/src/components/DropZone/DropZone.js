...
function DropZone() {
  ...
  const [waveformJson, setWaveFormJson] = useState([]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (artist === "") {
      ...
      formErrors["artist"] =
        "Artist name cannot be longer than 150 characters!";
    }
    if (title === "") {
    ...
    if (Object.values(formErrors).length > 0) {
      ...
      formData.append("waveformJson", JSON.stringify(waveformJson));
  ...
  const chooseFile = async (e) => {
    ...
    ws.current = WaveSurfer.create({ container: waveform.current });

    const arrBuff = await file.arrayBuffer();
    let blob = new Blob([new Uint8Array(arrBuff)], { type: file.type });
    ws.current.loadBlob(blob);

    ws.current.on("ready", () => {
      const peaks = ws.current.backend.getPeaks(300, 0, 300);
      setWaveFormJson(peaks);
      console.log(peaks);
    });
  };
        ...
        <div className="song-data">
          <div className="image-preview-div">
            ...
            <div className="image-upload-button">
              ...
              >
                <FontAwesomeIcon
                  className="fa-solid playlist-icon"
                  icon={faCamera}
                />
                Upload Image
              </label>
          ...
          <div className="song-submit-form">
            {!songLoading ? (
              <form onSubmit={handleSubmit} id="song-submit-form">
          ...
          <audio
            src={audioSource}
            onLoadedMetadata={getDurationAndTitle}
          ></audio>
        </div>
        <div className="button-hole">
          <p align="right">
            <button className="add-song-button cancel" onClick={handleCancel}>
              Cancel
            </button>
            <button
              type="submit"
              form="song-submit-form"
              className="add-song-button submit"
            >
              Save
            </button>
          </p>
      ...
      <div>
        ...
        >
          <div>
            ...
            <div className="upload-errors">{errors ? errors : null}</div>
          </div>
          <label
        ...
        <div id="upload-waveform" ref={waveform} />
      </div>
    );
  }
}
